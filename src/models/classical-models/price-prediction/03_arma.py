...
# Find the best combination (p, q) using train + validation.
# Train the final ARMA(p, q) model using only train and save it to joblib.
...
eval_pred_arma_path = os.getenv('OUT_EVAL_PRED_03_ARMA')  # Save the prediction (csv and image)
...
eval_pred_arma_path = Path(eval_pred_arma_path)
...
# 2. Search best ARMA(p, q) using validation set
# Better to use try and except because there are combinations that don't work

def search_best_pq(train, val, p_max = 5, q_max = 5):
    ...
    best_pq = min(mse_results, key = mse_results.get)
    ...
    y_vals = [k[1] for k in mse_results.keys()]
    z_vals = [v for v in mse_results.values()]

    plt.scatter(x_vals, y_vals, c = z_vals, cmap = 'inferno')
    plt.colorbar(label = 'MSE')
    plt.title("MSE for ARMA(p, q) — Validation Set")
    plt.xlabel("p")
    plt.ylabel("q")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(out_diverse_path / "03_arma_pq_vs_mse.png")
...
# 3. Train final ARMA model

def train_model(train, best_p, best_q):

    model = ARIMA(train, order = (best_p, 0, best_q)).fit()

    joblib.dump(model, out_model_path / '03_arma_model.joblib')
...
def evaluate_test(model, train, val, test, scaler_target):
    ...
    plt.plot(test.index, preds_eval, label = f'Prediction ARMA({model.model.order})', color = 'orangered')
    plt.title(f'Prediction vs Real — ARMA{model.model.order}')
    plt.xlabel("Date")
    plt.ylabel("SPY Close Price")
    plt.grid(True, alpha = 0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(eval_pred_arma_path / "ARMA_test_prediction.png")
...
def future_prediction(model, train, val, test, scaler_target, horizon = 30):
    ...
    future_df.to_csv(eval_pred_arma_path / 'future_30d_prediction_ARMA.csv')
    ...
    plt.plot(future_dates, preds_future_val, label = f'Prediction ARMA{model.model.order} +{horizon}d', color = 'orangered', marker = 'o')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(eval_pred_arma_path / 'future_30d_forecast_chart_ARMA.png')
...
if __name__ == "__main__":
    ...
    print("ARMA Model Pipeline...")

    # 1. Load data

    train, val, test, scaler_target = load_data()

    # 2. Optimal p-value and q-value

    best_p, best_q = search_best_pq(train, val)

    # 3. Model training
   
    model = train_model(train, best_p, best_q)
