...
eval_pred_gru_path = os.getenv('OUT_EVAL_PRED_18_GRU')  
...
eval_pred_gru_path = Path(eval_pred_gru_path)
...
# 2. Create multistep sequences for GRU input
...
class GRUModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        out, hs = self.gru(x)  # (batch_size, seq_length, hidden_size)
...
def train_gru_model(model, train_loader, val_loader, num_epochs, learning_rate, weight_decay, patience = 350):
    ...
    torch.save(model.state_dict(), dl_models_path / '18_gru_model.pth')

    return train_losses, val_losses

 
# 6. Evaluate the GRU model and graphics
...
def generate_plots(train_losses, val_losses, y_true, y_pred, train_true, train_pred, future_pred, dates_test, dates_future):
    ...
    plt.plot(train_losses, label = 'GRU Train Loss', color = 'royalblue', linewidth = 2)
    plt.plot(val_losses, label = 'GRU Validation Loss', color = 'orangered', linewidth = 2)
    plt.title('GRU Model: Loss over Epochs', fontsize = 28)
    plt.xlabel('Epoch', fontsize = 24)
    plt.ylabel('MSE Loss', fontsize = 24)
    plt.legend(fontsize = 18)
    plt.grid(True, alpha = 0.3)
    plt.savefig(eval_pred_gru_path / 'gru_loss_curve.png')
    plt.close()
    ...
    plt.legend(fontsize = 18)
    plt.grid(True, alpha = 0.3)
    plt.savefig(eval_pred_gru_path / 'gru_scatter.png')
    plt.close()
    ...
    plt.plot(valid_dates, y_pred[:, 0], label = 'GRU Forecast (t + 1)', color = 'springgreen', linewidth = 2, linestyle = '--')
    plt.title('GRU Test Set Performance: One-Step Ahead Forecast', fontsize = 28)
    plt.ylabel('SPY Price USD', fontsize = 24)
    plt.legend(fontsize = 18)
    plt.grid(True, alpha = 0.3)
    plt.xticks(rotation = 45)
    plt.tight_layout()
    plt.savefig(eval_pred_gru_path / 'gru_test_series.png')
    plt.close()
    ...
    plt.title('Training Set Fit (GRU Model Learning Capability)', fontsize = 28)
    plt.legend(fontsize = 18)
    plt.grid(True, alpha = 0.3)
    plt.savefig(eval_pred_gru_path / 'gru_train_fit.png')
    plt.close()
    ...
    plt.plot(future_x, future_pred, label = 'GRU Future Projection (30d)', color = 'red', marker = 'o', markersize = 4)
    ...
    plt.legend(fontsize = 18)
    plt.grid(True, alpha = 0.3)
    plt.savefig(eval_pred_gru_path / 'gru_future_forecast.png')
    plt.close()
...
if __name__ == '__main__':
    ...
    train_losses, val_losses = train_gru_model(model, train_dl, val_dl, epochs, learning_rate, weight_decay, patience)

    # 5. Model evaluation 

    model.load_state_dict(torch.load(dl_models_path / '18_gru_model.pth'))
    print("Evaluating GRU model on test set and generating plots...")
    ...
    print("--" * 25)
    print("GRU Model Metrics on Test Set:")
    print("GRU RMSE: ", round(rmse, 3))
    print("GRU MAE: ", round(mae, 3))
    print("GRU R2: ", round(r2, 3))
    ...
    pd.DataFrame({'Date': future_dates, 'Predicted': future_real}).to_csv(eval_pred_gru_path / 'gru_future_predictions.csv')
